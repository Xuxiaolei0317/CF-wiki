...
`Level`: 等级<br>
`Season`: 赛季<br>
`Mission Pass Type`: 0未解锁，对应r_level+1值解锁解锁<br>
...
`Time Limited Prize []`: 限时奖励<br>
    ...
    限时奖励:
        ...
        第三位为时间戳，触发限时奖励倒计时开始
`Chest Level`: mission pass宝箱等级<br>
`Chest Prize`: mission pass宝箱奖励<br>
`Boosters`: mission star booster的倒计时<br>
`Booster Multi`: mission star booster的倍率<br>
...
`level`: 等级<br>
`exp`: 经验值<br>
`mission_pass_plus_type`: 0未解锁，对应r_level+1值解锁<br>
`mission_pass_plus_prize_data`: 与普通Mission Pass数据一样<br>
`available_reward_data`: 与普通Mission Pass数据一样<br>
`chest_exp`: super pass 宝箱经验<br>
`chest_level`: super pass 宝箱等级<br>
`chest_prize_data`: super pass 宝箱奖励<br>
`last_mp_plus_type`: 上赛季 super pass 激活数据<br>
...
`total_purchase` 用户的总付费<br>
`max_purchase` 用户的最大付费<br>
